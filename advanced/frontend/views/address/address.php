	...
	<div class="main w1210 bc mt10">
		...
		<div class="content fl ml10">
			<div class="address_hd">
				<h3>收货地址薄</h3>
				<?php $count=0;foreach($models as $model):?>
					<?php $count++;?>
				<dl>
					<dt><?=$count.'、'?> <?=$model->username?> <?=$model->provinceaddress->name?> <?=$model->cityaddress->name?> <?=$model->countyaddress->name?> <?=$model->address?> <?=$model->tel?></dt>
					<dd>
						<?=\yii\helpers\Html::a('修改',['address/edit','id'=>$model->id])?>
						<?=\yii\helpers\Html::a('删除',['address/del','id'=>$model->id])?>
						<?=\yii\helpers\Html::a('设为默认地址',['address/set','id'=>$model->id])?>
					</dd>
				</dl>
				<?php endforeach;?>
...
<!--							<li>-->
								...
								<?php
									...
									echo $form->field($address,'username')->textInput(['class'=>'txt','value'=>'']);
...
//									echo $form->field($model,'county')->dropDownList(['prompt'=>'请选择'])->label(false);
								...
								echo $form->field($address, 'province')->widget(\chenkby\region\Region::className(),[
									...
									'district'=>[
								      ...
								      echo $form->field($address,'address')->textInput(['class'=>'txt','value'=>'']);
									  echo $form->field($address,'tel')->textInput(['class'=>'txt','value'=>'']);
